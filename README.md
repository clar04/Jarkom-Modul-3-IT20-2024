...
Buat .sh baru di semua laravelworker dengan script seperti berikut:
~~~
apt-get install git -y
...
echo 'server {
        ...
        error_log /var/log/nginx/atreides.log;
        access_log /var/log/nginx/atreides.log;

}' > /etc/nginx/sites-available/atreides

rm /etc/nginx/sites-enabled/atreides
ln -s /etc/nginx/sites-available/atreides /etc/nginx/sites-enabled/atreides
